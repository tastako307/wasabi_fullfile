...
trait Bitmap{
    ...
    /// # Safety
    /// 
    /// Returnedpointeris valid aslongas the given coordinates are valid
...
fn init_vram(efi_system_table: &EfiSystemTable) -> Result<VramBufferInfo> {
     ...
     Ok(VramBufferInfo {
        ...
        width: gp.mode.info.horizonal_resolution as i64,
...
impl<'a> Iterator for MemoryMapIterator<'a> {
    ...
    fn next(&mut self) -> Option<&'a EfiMemoryDescriptor> {
        ...
        //     &*(self.memory_map_buffer.as_ptr().add(self.current) as *const EfiMemoryDescriptor)
        // };
    }
}
